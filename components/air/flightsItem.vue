<template>
    <div class="flight-item">
        ...
        <div v-show="isshow" class="flight-recommend" v-for="(each,index) in item.seat_infos" :key="index">
            <!-- 隐藏的座位信息列表 -->
                ...
                <el-col :span="20">
                    <el-row type="flex" justify="space-between" align="middle" class="flight-sell">
                        <el-col :span="16" class="flight-sell-left">
                            <span>{{each.group_name}}</span> | {{each.supplierName}}
                        </el-col>
...
export default {
    ...
    computed: {
        ranktime(){
            const endarr=this.item.arr_time.split(':')
            let end=  endarr[0]*60 + Number(endarr[1])
            
             const deparr=this.item.dep_time.split(':')
              let dep=  deparr[0]*60 + Number(deparr[1])

            if(end<dep){
                end+=24*60
            }
              const hours= Math.floor((end-dep)/60)
